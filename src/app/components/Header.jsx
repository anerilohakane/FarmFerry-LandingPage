...
const Header = () => {
  ...
  const handleSelectProduct = useCallback((product) => {
    try {
      ...
      const categoryName = product?.categoryId?.name || 'Products';
      if (categoryId) {
        const url = `/products?category=${encodeURIComponent(categoryName)}&categoryId=${categoryId}`;
        router.push(url);
      } else if (product?._id) {
                    ...
                    {searchResults.map((product) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-gray-600 truncate">{product.unit}{product.categoryId?.name ? ` • in ${product.categoryId.name}` : ''}</div>
                        </div>
                      </div>
                    ))}
                  </div>
...
//           setUserLocation(data?.display_name ?? "Location not available");
//         } catch {
//           setUserLocation("Unable to fetch location");
//         }
//       },
...
//   const getToken = useCallback(() => {
//     try {
//       const saved = localStorage.getItem("farmferry-tokens");
...
//       }
//     },
//     [pathname, router, setCartOpen, setProfileOpen]
//   );
...
//   const redirectToPlayStore = useCallback(() => {
//     window.open(
//       "https://play.google.com/store/apps/details?id=com.farmferry.app",
//       "_blank"
//     );
//   }, []);
...
//     (msg) => setAlertModal({ isOpen: true, message: msg }),
//     []
//   );
//   const closeAlert = useCallback(
//     () => setAlertModal({ isOpen: false, message: "" }),
...
//       const timeout = setTimeout(async () => {
//         try {
//           const res = await apiService.searchProducts(term);
//           if (res?.success) {
...
//             setShowSearchDropdown(prods.length > 0);
//           } else {
//             setSearchResults([]);
//             setShowSearchDropdown(false);
//           }
...
//       const catName = product?.categoryId?.name ?? "Products";
//       if (catId) {
//         router.push(
...
//           </h3>
//           <p className="text-gray-600 text-xs sm:text-sm">{user?.phone || user?.email || 'User'}</p>
//         </div>
...
//                       handleSelectProduct(searchResults[0]);
//                   }}
//                   className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
//                 />
                
//                 {/* SEARCH DROPDOWN */}
//                 {showSearchDropdown && searchResults.length > 0 && (
//                   <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-lg shadow-lg max-h-80 overflow-y-auto z-50 mt-1">
//                     {searchResults.map((product) => (
//                       <div
//                         key={product._id}
//                         onClick={() => handleSelectProduct(product)}
...
//                 )}
//                 {showSearchDropdown && searchResults.length === 0 && searchTerm.length >= 2 && (
//                   <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-lg shadow-lg p-3 text-gray-600 mt-1">
//                     No products found for "{searchTerm}"
//                   </div>
//                 )}
//               </div>
...
//                 <button
//                   onClick={handleAuthAction}
//                   className="ml-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-full text-sm font-medium transition-colors shadow-sm whitespace-nowrap"
//                 >
//                   {isAuthenticated ? "Logout" : "Login"}
//                 </button>
//               </div>
...
//                     key={item.id}
//                     onClick={() => scrollToSection(item.id)}
//                     className="flex-shrink-0 text-gray-700 hover:text-green-600 px-2 py-1 text-xs transition-colors whitespace-nowrap"
//                   >
//                     {item.label}
//                   </button>
//                 ))}
...
//       {/* SPACER */}
//       <div className="h-[100px] sm:h-[90px] lg:h-[80px]" />
