  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-800 mb-4">Frequently Asked Questions</h1>
        <p className="text-base sm:text-lg text-gray-600">Find answers to common questions about FarmFerry</p>
      </div>

      {/* Category Filter */}
      <div className="flex flex-wrap gap-3 mb-10 justify-center">
        <button
          className={`px-4 py-2 text-sm sm:text-base rounded-full transition-all ${activeCategory === 'all' ? 'bg-green-700 text-white shadow-md' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
        ...
        {faqData.categories.map(category => (
          <button
            key={category.id}
            className={`px-4 py-2 text-sm sm:text-base rounded-full transition-all ${activeCategory === category.id ? 'bg-green-700 text-white shadow-md' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
        ...
        {filteredFAQs.length > 0 ? (
          filteredFAQs.map(item => (
            <div key={item.id} className="border border-gray-200 rounded-lg overflow-hidden transition-all hover:shadow-md">
              ...
              >
                <span className="font-medium text-base sm:text-lg text-gray-800">{item.question}</span>
                <svg
          ...
          <div className="text-center py-10">
            ...
            <h3 className="mt-4 text-base sm:text-lg font-medium text-gray-900">No FAQs found in this category</h3>
            <p className="mt-2 text-gray-500 text-sm sm:text-base">Try selecting a different category</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-16 p-6 bg-green-50 rounded-lg border border-green-200 text-center">
        <h3 className="text-lg sm:text-xl font-semibold text-green-800 mb-3">Still have questions?</h3>
        <p className="text-gray-700 mb-4 text-sm sm:text-base">Our support team is here to help you</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button className="px-6 py-3 bg-green-700 text-white rounded-lg hover:bg-green-800 transition-colors text-sm sm:text-base">
            Contact Support
          </button>
          <button className="px-6 py-3 border border-green-700 text-green-700 rounded-lg hover:bg-green-700 hover:text-white transition-colors text-sm sm:text-base">
            Live Chat
          </button>
        </div>
      </div>
